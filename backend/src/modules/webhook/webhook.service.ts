...
import { DepositWebhookDTO } from './webhook.schemas';

export async function processDeposit(dto: DepositWebhookDTO) {
  const token = dto.token.toUpperCase();
  if (!isSupportedToken(token)) {
    throw BadRequest('TOKEN_NOT_SUPPORTED', `Token não suportado: ${token}. Use BRL, BTC, ETH ou USDT.`);
  }
  ...
  if (existing) {
    const amountMatches = existing.amountTo ? toDecimal(existing.amountTo).equals(toDecimal(dto.amount)) : false;
    if (
  ...
  try {
    ...
    if (e instanceof Prisma.PrismaClientKnownRequestError && e.code === 'P2002') {
      ...
      if (ex) return { status: 'duplicate', transactionId: ex.id, message: 'race resolved' };
    }
    throw e;
  }
}
