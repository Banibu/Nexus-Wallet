...
import { Prisma, Movement, MovementType } from '@prisma/client';
...
export async function reconstructBalance(userId: string, token: string) {
  ...
  const balance = await prisma.balance.findUnique({
    where: { walletId_token: { walletId: wallet.id, token: token.toUpperCase() } },
  });
  return {
    token: token.toUpperCase(),
    reconstructed,
    stored: balance ? toDecimal(balance.amount).toString() : '0',
    consistent: balance ? toDecimal(balance.amount).eq(toDecimal(reconstructed)) : false,
  };
}
