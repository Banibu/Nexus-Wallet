...
import { toDecimal, toPrismaDecimal, roundForToken } from '../../utils/decimal';
import Decimal from 'decimal.js';
import { QuoteDTO, ExecuteDTO } from './swap.schemas';

function assertTokens(from: string, to: string) {
  if (!isSupportedToken(from)) throw BadRequest('TOKEN_NOT_SUPPORTED', `Token não suportado: ${from}`);
  if (!isSupportedToken(to)) throw BadRequest('TOKEN_NOT_SUPPORTED', `Token não suportado: ${to}`);
  if (from === to) throw BadRequest('SAME_TOKEN', 'fromToken e toToken devem ser diferentes');
}
...
export async function quote(dto: QuoteDTO): Promise<QuoteResult> {
  ...
  const feeAmount = roundForToken(amountOutGross.times(feePercent).div(100), dto.toToken);
  ...
  const result = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    ...
    const transaction = await tx.transaction.create({
      data: {
        ...
        metadata: { source: q.source, fetchedAt: q.fetchedAt, feePercent: q.feePercent },
      },
    });
