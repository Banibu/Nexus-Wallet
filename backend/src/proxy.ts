...
if (process.env.NODE_ENV === 'production') {
  ...
  if (fs.existsSync(frontendBuildPath)) {
    ...
    app.log.warn(`Frontend build directory not found at ${frontendBuildPath}. Skipping static file server.`);
  }
} else {
  // In development, proxy everything else to the Vite dev server (defaults to port 5173)
  const FRONTEND_DEV_TARGET = process.env.FRONTEND_DEV_TARGET || 'http://127.0.0.1:5173';
  app.register(httpProxy, {
    upstream: FRONTEND_DEV_TARGET,
    prefix: '/',
  });
  app.log.info(`Proxy Gateway configured in DEVELOPMENT mode. Proxying frontend requests to ${FRONTEND_DEV_TARGET}`);
}
