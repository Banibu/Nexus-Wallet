...
import { quote, execute } from '../modules/swap/swap.service';
...
describe('Nexus Wallet Services Tests', () => {
  ...
  describe('Deposit Webhook', () => {
    it('should process new deposit and credit balance', async () => {
      ...
      vi.mocked(prisma.$queryRaw).mockResolvedValueOnce([
        { id: 'bal-123', walletId: 'wallet-123', token: 'BRL', amount: new Decimal(0) },
      ]);
      vi.mocked(prisma.transaction.create).mockResolvedValueOnce({ id: 'tx-123' } as any);
      vi.mocked(prisma.movement.create).mockResolvedValueOnce({ id: 'mov-123' } as any);
  ...
  describe('Swap Service', () => {
    ...
    it('should execute swap and deduct balances', async () => {
      ...
      vi.mocked(prisma.wallet.findUnique).mockResolvedValueOnce(mockWallet as any);
      
      // Sorted alphabetically: BRL then BTC
      vi.mocked(prisma.$queryRaw)
        .mockResolvedValueOnce([
          { id: 'bal-brl', walletId: 'wallet-123', token: 'BRL', amount: new Decimal(0) },
        ])
        .mockResolvedValueOnce([
          { id: 'bal-btc', walletId: 'wallet-123', token: 'BTC', amount: new Decimal(2) },
        ]);
      vi.mocked(prisma.transaction.create).mockResolvedValueOnce({ id: 'tx-swap-123' } as any);
    ...
    it('should fail swap if balance is insufficient', async () => {
      ...
      vi.mocked(prisma.$queryRaw)
        ...
        .mockResolvedValueOnce([
          { id: 'bal-btc', walletId: 'wallet-123', token: 'BTC', amount: new Decimal(0.5) }, // insufficient
        ]);
  ...
  describe('Withdrawal Service', () => {
    it('should execute withdrawal if balance is sufficient', async () => {
      ...
      vi.mocked(prisma.wallet.findUnique).mockResolvedValueOnce(mockWallet as any);
      vi.mocked(prisma.$queryRaw).mockResolvedValueOnce([
        { id: 'bal-brl', walletId: 'wallet-123', token: 'BRL', amount: new Decimal(100) },
      ]);
      vi.mocked(prisma.transaction.create).mockResolvedValueOnce({ id: 'tx-withdraw-123' } as any);
      vi.mocked(prisma.movement.create).mockResolvedValueOnce({ id: 'mov-withdraw-123' } as any);
    ...
    it('should fail withdrawal if balance is insufficient', async () => {
      ...
      vi.mocked(prisma.wallet.findUnique).mockResolvedValueOnce(mockWallet as any);
      vi.mocked(prisma.$queryRaw).mockResolvedValueOnce([
        { id: 'bal-brl', walletId: 'wallet-123', token: 'BRL', amount: new Decimal(10) },
      ]);
